...
def parse_adherence(contract):
    ...
    key = "calc_" + \
        ast.literal_eval(config)["template"] + "_" + \
        str(contract.functions.get_hash().call())
    contracts = w3.eth.contract(address=contract_data[str(
        key)][2], abi=contract_data[str(key)][0], bytecode=contract_data[str(key)][1])
...
def represent_data(contract):
    ...
    for element in df_data.keys():
        if int(element) > days30:
            new_df_data[datetime.utcfromtimestamp(int(element)).strftime(
                '%Y-%m-%d %H:%M:%S')] = int(df_data[element])
    formatted = dict()
    formatted["Date"] = list(new_df_data.keys())
    formatted["Amount of Medication Taken"] = list(new_df_data.values())
    df = pd.DataFrame(formatted)
    fig = px.bar(df, x="Date", y="Amount of Medication Taken", text="Amount of Medication Taken",
                 color="Amount of Medication Taken", color_continuous_scale=px.colors.sequential.Reds)
    fig.update_layout(
        title="30 Day " + config["adherence"]["medication_name"].title() + " Compliance Report for " + config["first_name"].capitalize() +
        " " + config["last_name"].capitalize() +
        "(" + str(config["dob"]) + ")",
        font=dict(
            family="Geneva",
            size=17,
...
def calculate_adherence(contract):
    ...
    comp = (sum(days_7.values()) /
            (7 * (int(config["adherence"]["Times per day"]) + 1))) * 100
    ...
    with open("contract_data.json", "r") as infile:
        contract_data = json.load(infile)
    key = "calc_" + ast.literal_eval(contract.functions.get_config_file().call())[
        'template'] + "_" + str(contract.functions.get_hash().call())
    contracts = w3.eth.contract(address=contract_data[str(
        key)][2], abi=contract_data[str(key)][0], bytecode=contract_data[str(key)][1])
...
def daily_alerts(contract):
    ...
    if int(config["alerts"]["reminders"]) == 1:
        if int(config["adherence"]["Times per day"]) >= 3:
            schedule.every().day.at(scheduler[3][0]).do(
                send, to=config["communication"]["phone"], message="Good Morning!")
            ...
            schedule.every().day.at(scheduler[3][1]).do(
                send, to=config["communication"]["phone"], message="Good Afternoon!")
            ...
            for element in scheduler[int(config["adherence"]["Times per day"])]:
                schedule.every().day.at(element).do(
                    send, to=config["communication"]["phone"], message="Hi!")
...
def daily_summary(contract):
    ...
    today = str(int(datetime.today().timestamp()) -
                int(datetime.today().timestamp()) % 86400)
    if df_data[str(today)] == config["adherence"]["Times per day"]:
        send(config["communication"]["phone"],
             "You took all of your medications for today! Great Job!")
    if df_data[str(today)] < config["adherence"]["Times per day"]:
        send(config["communication"]["phone"],
             "Hey, you forgot to take your medication today! No worries- please remember to do it tomorrow :)")
...
def send_alert(event, contract):
    with open("contract_data.json", "r") as infile:
        contract_data = json.load(infile)
    config = ast.literal_eval(contract.functions.get_config_file().call())
    key = "calc_" + json.loads(contract.functions.get_config_file().call())[
        'template'] + "_" + str(contract.functions.get_hash().call())
    contracts = w3.eth.contract(address=contract_data[str(
        key)][2], abi=contract_data[str(key)][0], bytecode=contract_data[str(key)][1])
    compliance = contracts.functions.get_compliance().call()
    send("Hi! Your patient " + config["first_name"] + " " + config["last_name"] + "(" + str(config["dob"]) + ")" +
         "has only been " + str(compliance) + "%/ compliant in the last 7 days. Send him/her a reminder alert?")
    time.sleep(2)
